...
int main(int argc, char* argv[]) {
    ...
    if (config->print_version) {
        printf("Guacamole proxy daemon (guacd) - Cyolo build - 1.5.2-0 (standard recording)");
        exit(EXIT_SUCCESS);
    }
    ...
    /* Log start */
    guacd_log(GUAC_LOG_INFO, "Guacamole proxy daemon (guacd) - Cyolo build - 1.5.2-0 (standard recording)");
