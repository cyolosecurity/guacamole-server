...
int main(int argc, char* argv[]) {
    ...
    if (config->print_version) {
        printf("Guacamole proxy daemon (guacd) - Cyolo build - version " CYOLO_GUACD_VERSION);
        exit(EXIT_SUCCESS);
    }
    ...
    /* Log start */
    guacd_log(GUAC_LOG_INFO, "Guacamole proxy daemon (guacd) - Cyolo build - version " CYOLO_GUACD_VERSION);
